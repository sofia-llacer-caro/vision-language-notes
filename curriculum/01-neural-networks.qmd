...
3. Does backpropagation feel mysterious after reading Nielsen, or does it reduce to something familiar?

*(answers below)*

## My answers

1. A neural network is composed of individual processing units called neurons, organised in layers. Each neuron applies a linear transformation to its inputs (via weights and biases) followed by a nonlinearity such as a sigmoid. The nonlinearity is essential since, without it, stacking layers would collapse into a single linear transformation, losing all expressive power. Information flows forward through these layers, from input to output, where the final layer produces a prediction over some set of categories or values.

2. Training means adjusting the weights of the network to minimize a cost function, which measures the distance between the network's predictions and the correct answers in the training set. Geometrically, this is optimization in weight space: a space with as many dimensions as there are parameters in the network. Gradient descent walks downhill in this space by computing the gradient of the cost function with respect to all weights and taking a small step in the opposite direction. Backpropagation is what makes this efficient: it applies the chain rule recursively, propagating the error signal backward through the network and reusing intermediate gradients at each layer rather than recomputing them from scratch.

3. Not really, it reduces to the chain rule from calculus, applied recursively across the computational graph of the network.
